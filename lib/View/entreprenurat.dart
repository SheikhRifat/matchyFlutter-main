...
class Entreprenuriat extends StatefulWidget {
  const Entreprenuriat({ Key? key }) : super(key: key);

  @override
  State<Entreprenuriat> createState() => _EntreprenuriatState();
}

class _EntreprenuriatState extends State<Entreprenuriat> {
  List Matches=[];
  final controllerr=SwipableStackController();
  List DLike=[];
  List Like=[];
  Timer? timer;
String? nn,currentNom,image;
 GetUser()async{
...
void MatchUserEntrepren() async {
  ...
  await FirebaseFirestore.instance.collection('Users').where("objectifs",arrayContainsAny: ["investir dans des projets"
  ,"Trouver des investisseur",] )
  .get().then((val) {
    setState(() {  });
...
print(e);
  ...
  void initState() {
        super.initState();
     timer = new Timer.periodic(Duration(seconds: 2), (Timer t) =>   MatchUserEntrepren());
